...
class CombinedTracker:
    ...
    def __init__(self):
        ...
        if self.is_live:
            ...
            self.stream = VideoStream(self.video_path)
        ...
        self.current_unprocessed_frame = None  # Store raw Bayer frame from camera thread
    ...
    def frame_reader_thread_method(self):
        ...
        while self.running:
            ...
            if self.is_live:
                ...
                unprocessed_frame, metadata = self.camera.get_frame()
                ...
                with self.result_lock:
                    self.current_unprocessed_frame = unprocessed_frame
            else:
                ret, unprocessed_frame = self.stream.read()
                t_read = time.perf_counter()

                if not ret or unprocessed_frame is None:
                    continue
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                with self.result_lock:
                    if self.current_unprocessed_frame is None:
                        continue
                    unprocessed_frame = self.current_unprocessed_frame.copy()
                ...
                if self.use_gpu_processing:
                    try:
                        frame = self.gpu_preprocessor.process_frame(unprocessed_frame)
                    except Exception as e:
                        print(f"\nGPU processing failed, falling back to CPU: {e}")
                        self.use_gpu_processing = False
                        frame, _ = self.cpu_preprocessor.process_frame(unprocessed_frame)
                else:
                    frame, _ = self.cpu_preprocessor.process_frame(unprocessed_frame)
